...
class PROControl(nn.Module):
    ...
    def update_weights(self, stimuli, response, actual_ro, reward, valence, gating_signal):  # Added target_valence and gating_signal
        ...
        # -------------------------- handled in backprop/loss
        # # Update R-O conjunction weights (W_S)
        # ro_error = (self.theta * actual_ro - ro_predictions)
        # self.W_S.data += gating_signal * torch.outer(effective_lr * ro_error, stimuli)

        # # Update temporal prediction weights (U)
        # temporal_prediction = self.compute_temporal_prediction()
        # next_temporal = self.compute_temporal_prediction()  # Calculate next temporal prediction
        # td_error = reward + self.gamma * next_temporal.detach() - temporal_prediction  # Detach next_temporal
        # self.U.data += self.alpha_td * td_error.view(-1, 1, 1) * self.eligibility_trace
...
class GoNoGoTrainer:
    ...
    def run_trial(self, stimulus, correct_response, is_go_trial=None):
        ...
        # Reward and Valence (task-specific)
        reward, valence = self.get_reward_valence(eval_results, is_go_trial)

        # Calculate targets
        target_valence = reward + self.model.gamma * self.model.compute_temporal_prediction()
        target_valence = target_valence.detach() # Detach to prevent gradients flowing back through temporal prediction pathway



        # Update weights (and get losses)
        update_results = self.model.update_weights(stimulus, response.detach(), actual_outcomes, reward, valence, gating_signal=1.0)

        # Compute losses (using update_results and target_valence)
        losses = self.model.compute_losses(response, pred_outcomes, pred_temporal, actual_outcomes, reward, target_valence)

        # Backward pass
        losses['total_loss'].backward()  # Backpropagate total loss
        self.optimizer.step()
    ...
    def get_reward_valence(self, eval_results, is_go_trial):
        ...
        if isinstance(self.task, GoNoGoTask):
            ...
            reward = 1.0 if eval_results['correct'] else -1.0  # Example
            valence = torch.tensor([reward], device=device).float()
            return reward, valence
        elif isinstance(self.task, ForagingTask):
            # Foraging Task reward/valence
            response_idx = torch.argmax(eval_results['response_made']).item()
            if response_idx == 0:  # Engage
                reward = torch.tensor(np.random.choice(self.task.engage_values), device=device).float()
            else:  # Forage
                new_engage_values = np.random.choice(self.task.forage_values, size=2, replace=False)
                reward = torch.tensor(np.mean(new_engage_values) - self.task.forage_cost, device=device).float()

            valence = reward.clone().detach() # Detach valence
            return reward, valence
        elif isinstance(self.task, RiskAvoidanceTask):
            response_idx = torch.argmax(eval_results['response_made']).item()
            if response_idx == 0:  # Safe
                reward = 0.5  # Example safe reward
            else:  # Risky
                reward = 1.0 if np.random.rand() < self.task.risky_win_prob else -1.0
            reward = torch.tensor([reward], device=device).float()
            valence = reward.clone().detach()
            return reward, valence
        elif isinstance(self.task, CorrelatedOutcomesTask):
            reward = 1.0 if eval_results['response_made'][self.task.current_correct_option] == 1 else -1.0
            reward = torch.tensor([reward], device=device).float()
            valence = reward.clone().detach()
            return reward, valence
        else:
            return 0.0, torch.tensor([0.0], device=device).float()
