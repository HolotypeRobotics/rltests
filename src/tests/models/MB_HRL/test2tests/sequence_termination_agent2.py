...
class MultiSeqEnv:
    ...
    def _get_obs(self):
        ...
        # (Note: previous action and head direction are maintained externally.)
        # print(f"{pos_onehot}")
        # print(f"{self.head_direction}")
        # input()
...
class Agent(nn.Module):
    ...
    def forward(self, position, direction, prev_option, belief=None):
        ...
        if x.dim() == 2:
            x = x.unsqueeze(1)
        out, _ = self.gru(x)  # out shape: (batch, 1, hidden_dim)
        h = out.squeeze(1)    # (batch, hidden_dim)
    ...
    for episode in range(num_episodes):
        ...
        while not episode_done:
            ...
            while not low_done and not task_switch:
                ...
                target_index = torch.argmax(position, dim=1)
                ...
                low_loss = low_policy_loss + low_value_loss + pos_loss_weight * low_total_state_loss + info_loss_weight * low_info_loss
            ...
            # TODO: calibrate the confidence for the high prediction varaibles
            high_info_gain = torch.abs(high_conf - low_conf.detach())
            high_info_loss = F.mse_loss(high_info_value, high_info_gain)
            
            high_loss = high_policy_loss + high_value_loss + pos_loss_weight * high_total_state_loss + info_loss_weight * high_info_loss
