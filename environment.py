...
class Environment:
    ...
    def plot_path(self, agent):
        ...
        for i in range(self.rewards.shape[0]):
            for j in range(self.rewards.shape[1]):
                ...
                ax.text(j, i, f"{cell_value:.4f}", ha='center', va='baseline', color='black')
    ...
    def step(self, action):
        ...
        if self.state == self.exit:
            ...
            self.state = (min(self.state[0] + 1, self.rewards.shape[0] - 1), self.state[1])

        elif action == Action.LEFT:
            self.state = (self.state[0], max(self.state[1] - 1, 0))

        elif action == Action.RIGHT:
            self.state = (self.state[0], min(self.state[1] + 1, self.rewards.shape[1] - 1))
        ...
        return np.array(self.state_to_sdr(), dtype=int), self.reward, self.done, self.state
